...
namespace CRRT
{
	...
	Mesh PrimitiveMeshFactory::CreateACube()
	{
		float vertices[] =
		{
			//				 x	    y       z     w       r		g	  b    a		u    v		     nx	   ny     nz	
	...
	Mesh PrimitiveMeshFactory::CreateASphere(unsigned int sector_count, unsigned int span_count)
	{
		std::vector<Vertex> vertices;
		std::vector<unsigned int> indices;
	...
	//template<class T>
	Mesh PrimitiveMeshFactory::CreateAQuad()
	{
		////For cal Tangent & Bitagent
		glm::vec3 p0(-0.5f, 0.5f, 0.0f),
			p1(-0.5f, -0.5f, 0.0f),
